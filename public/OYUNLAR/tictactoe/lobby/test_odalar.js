...
// Version: v6 - Async/Auth Flow Fix (Yönləndirmə düzəlişi ilə)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupSocketConnection() {
        ...
        socket.on('roomList', (roomsFromServer) => {
            console.log('[DEBUG] roomList alındı, Otaq sayı:', roomsFromServer?.length ?? 0);
            currentRooms = {};
            if (Array.isArray(roomsFromServer)) {
                roomsFromServer.forEach(room => { if (room && room.id) { currentRooms[room.id] = room; } else { console.warn("[DEBUG] Keçərsiz otaq datası:", room); } });
                displayRooms(Object.values(currentRooms));
            } else { console.error("[DEBUG] roomList: Array gözlənilirdi!", roomsFromServer); displayRooms([]); }
        });
        ...
        // Otaq silinməsində xəta olduqda
        ...
        // Otağa uğurla qoşulduqda (serverdən 'room_joined' gəlir)
      // Otağa uğurla qoşulduqda (serverdən 'room_joined' gəlir)
      socket.on('room_joined', (data) => {
        // >>> LOBBY DEBUG: room_joined ALINDI <<<
        console.log('[LOBBY-DEBUG] <<< room_joined hadisəsi alındı:', data);
        // -----

        if (!data || !data.roomId) {
             console.error('[LOBBY-DEBUG] room_joined datası natamamdır!', data);
             alert('Otağa qoşulma məlumatları natamamdır.');
             return;
        }
        ...
        hideModal(createRoomModal); // Yaratma modal pəncərəsini də bağla (ehtiyat üçün)

        // Oyun səhifəsinə yönləndir
        const params = new URLSearchParams({
            roomId: data.roomId,
             // Əlavə olaraq roomName və size-ı da göndərə bilərik, oyun səhifəsi üçün faydalı ola bilər
             roomName: encodeURIComponent(data.roomName),
             size: data.boardSize // boardSize server tərəfindən room_joined içində göndərilməlidir
        });
         // Düzgün URL (kök qovluqdan başlayan)
        const gameUrl = `/OYUNLAR/tictactoe/game/oda_ici.html?${params.toString()}`;

         // >>> LOBBY DEBUG: Yönləndirmə baş verir <<<
         console.log(`[LOBBY-DEBUG] Oyun səhifəsinə yönləndirilir: ${gameUrl}`);
         console.log(`[LOBBY-DEBUG] window.location.href təyin edilir...`);
         // -----
        try {
             window.location.href = gameUrl;
             // >>> LOBBY DEBUG: Yönləndirmə əmri verildi <<<
             console.log(`[LOBBY-DEBUG] window.location.href təyin edildi. Yönləndirmə baş verməlidir.`);
             // -----
        } catch (e) {
             console.error("[LOBBY-DEBUG] Yönləndirmə zamanı xəta:", e);
             alert("Oyun otağına keçid zamanı xəta baş verdi.");
        }
    });
        // ===> DÜZƏLİŞ SONU <===
    ...
    function attachLobbyEventListeners() {
        ...
        if (createRoomButton && createRoomModal) { createRoomButton.addEventListener('click', () => { console.log("[DEBUG] 'Yeni Oda Oluştur' düyməsinə klikləndi!"); const nameInput = document.getElementById('new-room-name'); const passInput = document.getElementById('new-room-password'); const sizeSelect = document.getElementById('new-board-size'); if(nameInput) nameInput.value = ''; if(passInput) passInput.value = ''; if(sizeSelect) sizeSelect.value = '3'; if(createRoomMessage) createRoomMessage.textContent = ''; createRoomMessage.className = 'message'; if(createRoomSubmitBtn) createRoomSubmitBtn.disabled = false; updateRoomRuleDisplay(); showModal(createRoomModal); }); console.log("[DEBUG] 'Yeni Oda Oluştur' düyməsinə listener qoşuldu."); } else { console.error("[DEBUG] 'Yeni Oda Oluştur' düyməsi və ya modal pəncərə DOM-da tapılmadı!"); }
    ...
    try {
        ...
        const response = await fetch('/api/auth/check-auth'); // credentials: 'include' artıq lazım deyil (əgər session cookie httpOnly deyilsə)
        ...
        if (!response.ok || !data.loggedIn || !data.user) {
            ...
            return; // Yönləndirmədən sonra kodu dayandır
        }

        loggedInUser = data.user;
        console.log(`[DEBUG] Lobby JS: Autentifikasiya uğurlu: ${loggedInUser.nickname}`);
        if (welcomeText) welcomeText.innerHTML = `Xoş gəldin, <strong>${escapeHtml(loggedInUser.nickname)}</strong>! Oyuna qatılmaq üçün otaq seçin və ya yenisini yaradın.`;
        ...
        // UI-da xəta göstərmək üçün əlavə kodlar
    }
